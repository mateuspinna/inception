...
echo "Installing and starting WordPress"
        wget -q -O - https://wordpress.org/latest.tar.gz | tar -xz -C /var/www/html/wp --strip-components=1
        chmod -R +rwx /var/www/html/wp

        wp --path=/var/www/html/wp --allow-root config create --dbname="$SQL_DATABASE" --dbuser="$SQL_USER" --dbpass="$SQL_PASSWORD" --dbhost="mariadb":"3306" --dbprefix='wp_'
